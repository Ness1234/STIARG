...
toFrame :: (Int, Int) -> (Int, Int) -> [(Int, Int)] -> [(Int, Int)] -> [(Int, Int)] -> [(Int, Int)] -> Int -> Int -> ListFrame
toFrame (xdim, ydim) (x', y') xs ys zs as c speed = ListFrame $ map (\y -> map (\x -> draw x y xs ys zs as c speed) [0 .. xdim - 1]) [0 .. ydim - 1]

draw :: Int -> Int -> [(Int, Int)] -> [(Int, Int)] -> [(Int, Int)] -> [(Int, Int)] -> Int -> Int -> Pixel
...
newFrame ints events state@(State playerPosition enemies tick speed speedTick) | (enemyHit enemies playerPosition)  = (toFrame dim playerPosition' (myCar playerPosition') (myTire playerPosition') (mergeLists (map otherCar enemypositions)) (mergeLists (map otherTire enemypositions)) tick speed, (State (1, 6) [] 25 15 0))
                                        ...
                                        where
                                            newSpeed | tick == 500 && speed /= 1 = speed - 1
                                                     | otherwise                 = speed
                                            newSTick | speedTick == speed = 0
                                                     | otherwise = speedTick + 1
                                            playerPosition' = foldl (\acc (Event mod ev) -> if mod == "KEYBOARD" then move ev acc else acc) playerPosition events
                                            enemyList | ((ints !! 1) `mod` 10) == 1 = enemies ++ (withoutConflict enemies ints)
                                                      | otherwise                   = enemies  
                                            newTick | tick == 0 = 500
                                                    | otherwise = tick - 1
                                            enemypositions | speedTick == 0 = map moveEnemy enemyList
                                                           | otherwise               = enemyList
